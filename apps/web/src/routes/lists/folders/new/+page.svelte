<script lang="ts">
	...
	async function save() {
		...
		saving = true;
		try {
			await createFolder({ name: name.trim(), color });
...
<main class="mx-auto flex max-w-lg flex-col gap-4 p-8">
	<PageHeader title="New Folder" backHref={resolve('/lists')} backLabel="Cancel">
		{#snippet actions()}
	...
	<form class="flex flex-col gap-4" onsubmit={handleSubmit}>
		<div class="flex flex-col gap-1">
			...
			<Input id="new-folder-name" placeholder="Folder name" bind:value={name} autofocus />
		</div>
