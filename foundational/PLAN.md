...
Status: **In progress — Phase 0 (Foundations) and Phase 1 (Auth & domain core) complete; Phase 2 (List & item CRUD) functionally complete end-to-end, testing/polish gaps remain (see the status note at the end of this document). Phase 3 (Sharing & real-time) not started.**
...
- *Known gaps, in rough priority order: (1) no component-level tests for any of the new pages — the existing `vitest-browser-svelte` browser-mode test project also can't run in this sandbox (pre-existing Playwright browser-binary version mismatch, unrelated to this work; confirmed by reproducing it on the pre-Phase-2 commit too), so this couldn't be verified here either way; (2) `apps/web`'s Vitest coverage thresholds (100%, per §11) still aren't wired up — `apps/api` now has its gate (see above) but the frontend doesn't, partly blocked on (1); (3) the store/category/favorites screens were checked with `svelte-check`/`eslint`/`prettier`/a production build plus the backend's Japa coverage, but not manually clicked through in a real browser against a running API; (4) category and store icon fields are plain text inputs — the searchable `@mdi/js` icon picker described in §4/§7 isn't built.*
