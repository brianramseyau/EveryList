...
- *Frontend done: login/signup, a list index with create, and a list detail page (add item, check/uncheck grouped by category, paste import, remove) — enough to use the app end-to-end for the core loop. Wired through a small `$lib/api/*` client (bearer token in localStorage, `@everylist/shared` DTOs). Because `/lists/[id]` has params unknown at build time, `apps/web` now builds with adapter-static's `fallback: '200.html'` instead of being fully prerendered, and `apps/api/start/routes.ts` grew a catch-all route to serve that shell for direct navigation/refresh on such routes (verified by hand against a built image: `/`, `/lists`, `/lists/:id`, and unmatched `/api/v1/*` all resolve correctly). Component-level tests for the new pages are not written yet — the existing `vitest-browser-svelte` browser-mode test project also can't run in this sandbox (pre-existing Playwright browser-binary version mismatch, unrelated to this work; confirmed by reproducing it on the pre-Phase-2 commit too) so that gap couldn't be verified here either way.*
- *Not started: category customization UI, store selector / "reorder categories for this store" screen, favorites UI, recent-items restore UI. Needed before Phase 2 is fully demoable end-to-end, not just for the core add/check/import loop.*
