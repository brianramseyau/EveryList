...
Status: **In progress — Phase 0 (Foundations) and Phase 1 (Auth & domain core) complete; Phase 2 (List & item CRUD) complete, including component tests, coverage gates on both `apps/api` and `apps/web`, and the `@mdi/js` icon picker (a couple of minor polish items remain — see the status note at the end of this document). Phase 3 (Sharing & real-time) not started.**
...
- *Known gaps, in rough priority order: (1) the store/category/favorites screens were checked with `svelte-check`/`eslint`/`prettier`/a production build plus the new component-test suite, but not manually clicked through in a real browser against a running API end-to-end; (2) frontend coverage is at a real ~90% floor, not the eventual 100% — see above; (3) the icon picker's capped/search-gated grid stands in for true virtualization; (4) the PWA/service-worker precaching of the icon-picker chunk described in §4 depends on Workbox/`vite-plugin-pwa` infrastructure that hasn't been built yet (tracked separately, not a Phase 2 regression — no service worker exists in the app at all yet).*
