...
- *Known gaps, in rough priority order: (1) no component-level tests for any of the new pages — the existing `vitest-browser-svelte` browser-mode test project also can't run in this sandbox (pre-existing Playwright browser-binary version mismatch, unrelated to this work; confirmed by reproducing it on the pre-Phase-2 commit too), so this couldn't be verified here either way; (2) the store/category/favorites screens were checked with `svelte-check`/`eslint`/`prettier`/a production build plus the existing backend Japa coverage, but not manually clicked through in a real browser against a running API; (3) category and store icon fields are plain text inputs — the searchable `@mdi/js` icon picker described in §4/§7 isn't built; (4) the 100% coverage policy in §11 hasn't been re-audited since Phase 2's backend additions.*
