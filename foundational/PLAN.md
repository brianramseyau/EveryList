...
Status: **In progress — Phase 0 (Foundations), Phase 1 (Auth & domain core), and Phase 2 (List & item CRUD) complete, including component tests, coverage gates on both `apps/api` and `apps/web`, the `@mdi/js` icon picker (categories), and a manual end-to-end smoke pass against a live API (a couple of minor polish items remain — see the status note at the end of this document). Phase 3 (UI/UX Redesign & Theming Foundations) complete — see §16 and the status note. Phase 4 (Sharing & real-time) not started.**
...
- *Testing: every new/changed component and page got test coverage in the same pass (`ColorPicker.svelte.spec.ts`, `PageHeader.svelte.spec.ts`, updated fixtures adding `itemCount` to every hardcoded `ListDto` literal across the existing page specs, new assertions for the item-count meta line, the category-header accent color, and the Settings logout flow) plus a new backend functional-test path exercising `itemCount` through create → add item → check item → verify it drops out of the count. `apps/api` stayed at 100% coverage; `apps/web`'s existing floor-gate (not yet the eventual 100%, see the Phase 2 status note above) still passes with room to spare (136 tests, ~91% statements/lines, ~91% functions, ~76% branches). Both workspaces are lint- and typecheck-clean.*
