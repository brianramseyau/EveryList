...
Status: **In progress — Phase 0 (Foundations), Phase 1 (Auth & domain core), and Phase 2 (List & item CRUD) complete, including component tests, coverage gates on both `apps/api` and `apps/web`, the `@mdi/js` icon picker, and a manual end-to-end smoke pass against a live API (a couple of minor polish items remain — see the status note at the end of this document). Phase 3 (Sharing & real-time) not started.**
