...
Status: **In progress — Phase 0 (Foundations), Phase 1 (Auth & domain core), and Phase 2 (List & item CRUD) complete, including component tests, 100% coverage gates on both `apps/api` and `apps/web`, a truly virtualized `@mdi/js` icon picker (categories), and a manual end-to-end smoke pass against a live API. Phase 3 (UI/UX Redesign & Theming Foundations) complete — see §16 and the status note. Phase 4 (Sharing & real-time) complete — see the status note. Phase 5 (Offline & PWA) not started.**
