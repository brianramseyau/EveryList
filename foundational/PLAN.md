...
Status: **In progress — Phase 0 (Foundations), Phase 1 (Auth & domain core), and Phase 2 (List & item CRUD) complete, including component tests, coverage gates on both `apps/api` and `apps/web`, the `@mdi/js` icon picker (categories), and a manual end-to-end smoke pass against a live API (a couple of minor polish items remain — see the status note at the end of this document). Phase 3 (UI/UX Redesign & Theming Foundations) started — see §16 and the status note. Phase 4 (Sharing & real-time) not started.**
...
| Smart autocomplete & auto-categorization | Free | **MVP** (basic) | Seeded category keyword matching; ML-personalized suggestions deferred to Phase 7. |
...
| Item photos | Premium | **Phase 6** | Needs object storage; deferred, not architecturally hard. |
...
| File storage | **Local filesystem** (`/config/uploads`) | Item photos, Phase 6. Kept behind a storage-service interface so swapping in an S3-compatible backend later is a config change, not a rewrite. |
...
**Deployment shape:** a single Docker image, built on an s6-overlay base image (mirroring LinuxServer.io's own pattern), so `PUID`/`PGID` environment variables control filesystem ownership exactly the way LSIO images do — the init stage remaps the container's app user to the supplied IDs and `chown`s `/config` before dropping privileges and starting the app. Inside the container there is exactly **one Node process**: AdonisJS serves both the REST/SSE API and the static SvelteKit build, so the container exposes exactly one HTTP port and needs no internal reverse proxy. All persistent state — the SQLite database file and, from Phase 6, uploaded item photos — lives under a single `/config` volume, which also makes Unraid appdata-backup plugins trivial to use for backups. This is a single-instance deployment target by design; no clustering/multi-node concerns apply (see the in-memory rate-limit store and Transmit local transport in §4).
...
- **Item** — id, listId, name, quantity, notes, categoryId, checked, checkedAt, sortOrder, storeId (nullable, Phase 6 — item-to-store tagging), price (nullable, Phase 6), photoUrl (nullable, Phase 6), createdBy, timestamps + `deletedAt` (backs Recent Items recovery).
...
| **6 — Item-store tagging, prices, folders, photos, export** | Item-to-store tagging + filtered "show only this store's items" view (on top of the `Store` entity from Phase 2), price/budget tracking, list folders, item photos (local filesystem storage), badge counts/exclusion, email export. |
...
2. **App shell** (started — bottom nav done, header still per-page): persistent bottom navigation across the authenticated Lists/Favorites/Settings sections; a shared header treatment (title + contextual actions) to replace the one-off `<h1>` + inline links each page currently rolls itself.
3. **List index redesign:** each list becomes a card-style row with an icon/color swatch (backed by `List.color`/`List.icon`, already in the §7 schema — Phase 2 just never surfaced them in the UI), item-count meta line, and a tap target sized for mobile, replacing the current bare bordered `<a>` row.
4. **Category headers redesign:** section headers on the list-detail screen gain the category's icon and accent color instead of a plain gray caption, matching how `Category.icon` (§7) is already stored but not yet rendered anywhere in the frontend.
5. **Icon pickers:** category create/rename already got the searchable `@mdi/js` picker (`Icon.svelte`/`IconPicker.svelte`) as part of Phase 2's late polish pass — landed ahead of this phase. What's left: wire the same picker into the store-create form (still a plain text `icon` input) and add a color/icon picker for lists (`List.color`/`List.icon` exist in the schema but have no UI at all — not even at list creation).
6. **Settings restructure:** group the single paragraph into labeled sections (Account, Appearance, About/build info), starting from the "App Theme" section added in this phase's first slice.
...
- *Known gaps, in rough priority order: (1) frontend coverage is at a real ~90% floor, not the eventual 100% — see above; (2) the icon picker's capped/search-gated grid stands in for true virtualization; (3) the PWA/service-worker precaching of the icon-picker chunk described in §4 depends on Workbox/`vite-plugin-pwa` infrastructure that hasn't been built yet (tracked separately, not a Phase 2 regression — no service worker exists in the app at all yet).*

*Phase 3 — UI/UX Redesign & Theming Foundations — started (see §16 for the full brief); the theming half is functionally done, the visual-redesign half is mostly not:*
- *Done: class-based dark mode (`@custom-variant dark` over Svelte's `.dark` root class, not just the `prefers-color-scheme` media query Phase 1/2 pages leaned on piecemeal), a `light`/`dark`/`automatic` preference persisted to `localStorage` and applied via `$lib/theme.ts`, a blocking inline bootstrap script in `app.html` so there's no flash of the wrong theme on load, an "App Theme" control on the Settings page (first grouped-row settings section, §16), and app-wide `bg`/`text` tokens on the root layout so the many pre-existing `dark:text-*` utilities from Phase 1/2 now actually sit on a matching dark background instead of white. A minimal persistent bottom navigation (Lists / Favorites / Settings) was added to the root layout, shown whenever a token is present and the route is under one of those three sections — the first piece of the AnyList-style app shell described in §16. The list index also got a first pass at card-style rows with a color swatch pulled from the `List.color` field that already existed in the schema but was never rendered. The category `@mdi/js` icon picker (§16 deliverable 5) landed separately as part of the Phase 2 polish pass above — that half of the deliverable is already done, ahead of the rest of Phase 3.*
- *Not done yet, left for the rest of Phase 3: list rows still have no icon rendering or item-count meta line (the latter needs an API addition — `ListDto` doesn't carry a count — so it's explicitly parked for a future slice, not silently dropped); category-header visual redesign (icon/accent color on section headers, even though `Category.icon` now resolves via the real picker/`Icon.svelte`, the list-detail screen still renders the header as plain gray text); the store-create form's `icon` field is still a plain text input — the picker only reached categories; there is no icon/color picker for lists at all (`List.color`/`List.icon` are set at creation time with no UI); and a broader settings restructure beyond the one new "Appearance" section. None of this blocks Phase 4.*
