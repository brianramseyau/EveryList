...
- **Credentials via an in-app auto-minted PAT.** The app's existing Personal Access Token
  ...
  named "Home-screen widget" and hands it to the widget over a deep link. The user picks lists
  in-app; the widget configures itself. No typing tokens/URLs by hand.
...
- **`src/domain.ts`**: add `WidgetConfigDto { token: string, listIds: number[], serverUrl: string }`
  — the payload the web app passes to the native widget over the deep link. Export from `index.ts`.
...
1. **Deep-link support** (`@capacitor/app` is already a dependency):
   ...
   - In `apps/web/src/routes/+layout.svelte`, alongside the existing startup logic, add an
     ...
     - `everylist://widget-config?token=...&listIds=...&serverUrl=...` → `goto` the widget config
       page.
2. **`apps/web/src/lib/widget.ts`** — `configureWidget(listIds)`:
   ...
   - Build a `everylist://widget-config?...` deep link carrying `token`, `listIds`, and
     `getServerUrl()` from `lib/api/server-url.ts`.
   - Open it with `@capacitor/app`'s `App.openUrl` (or `Browser`). No-op / guarded when not native.
...
2. **`WidgetConfigActivity.java`** — the config screen (`android:configure` on the provider info):
   - Parses the deep-link payload (`token`, `listIds`, `serverUrl`) handed from the app.
   - Lets the user choose which granted list to display and the show/hide-completed default.
   - Persists credentials + selected list to `SharedPreferences`, scoped to the widget instance.
3. **`WidgetApiClient.java`** — plain HTTP calls against `${serverUrl}` with
   `Authorization: Bearer <token>`:
   - `GET /api/v1/lists` (selector), `GET /api/v1/lists/<id>/items` (rows), `POST /api/v1/lists/<id>/items`
     (quick-add), `PATCH /api/v1/lists/<id>/items/<itemId>` (check/uncheck). Never runs on the main
     thread.
4. **`WidgetUpdateService.java`** — a `Service` target for the widget's pending intents, so
   checkbox taps and refresh happen off the main thread.
5. **Layouts** (`res/layout/widget_everylist.xml`):
...
6. **`res/xml/everylist_widget_info.xml`** — `AppWidgetProviderInfo` (`minWidth/minHeight`, preview
...
8. **Manifest:** register the provider (`<receiver android:name=".EveryListWidget">` +
   `APPWIDGET_UPDATE` intent-filter + metadata), the config activity, and the custom-scheme
   intent-filter for the deep link (shared with the web side above).
...
- **web** (Vitest, 100% gate): `widget.ts` (mint + deep-link building, native-only guards) and the
  `+layout.svelte` deep-link routing branches.
- **android** (JUnit under `apps/android/app/src/test/java/...`): `WidgetApiClient` URL/parsing/
  error handling; deep-link payload parsing in `WidgetConfigActivity` (extracted into a pure
  testable helper).
...
3. **Part 2** — web deep links + `widget.ts` + Settings entry + widget config page.
