 ...
 * dictionary keyed by default category name (see apps/api's
 * database/seeders/default_category_seeder.ts). Shared between the API
 * (server-side confirmation on item create) and the web client's offline
 * optimistic insert, so both sides agree on the guess without a round trip
 * — see PLAN.md §9. ML-personalized suggestions are deferred to Phase 7
 * (PLAN.md §3).
 */
