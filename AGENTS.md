...
- **Cutting a release (`vX.Y.Z`)** — two steps, in this order; nothing here is automated and `main` is PR-only, so each version change goes through a branch + PR (see the PR review loop below):
  ...
  - **HA add-on, automated.** The Home Assistant Supervisor add-on lives in its own repo, [`everylist-ha-app`](https://github.com/brianramseyau/everylist-ha-app) — no manual step here. Once `docker-publish.yml`'s `merge` job confirms the tag's image is fully published, its `bump-ha-addon` job authenticates as a GitHub App installation (`HA_ADDON_APP_ID`/`HA_ADDON_APP_PRIVATE_KEY`, installed only on that repo — a real identity, not `GITHUB_TOKEN`, so the PR it opens actually fires `pull_request` checks there) and opens a PR bumping `everylist/config.yaml`'s `version` and appending a link-only `CHANGELOG.md` entry, then auto-merges it once that repo's `validate` check passes. Watch it land (`gh pr list --repo brianramseyau/everylist-ha-app`) for the first few releases rather than assuming it worked. If the job fails (e.g. the App's key needs rotating), it's safe to fall back to running its logic by hand: clone `everylist-ha-app`, `node .github/scripts/bump-version.mjs vX.Y.Z`, review, commit/PR/merge.
